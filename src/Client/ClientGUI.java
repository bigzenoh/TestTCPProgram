...
public class ClientGUI extends javax.swing.JFrame {
    ...
    private void joinBtnActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        ClientGUI.ReceivingThread t = new ClientGUI.ReceivingThread(client.getClientSocket(), this);
        t.start();
        //xoá nút start
    ...
    public Object[] resultRow(){
        ...
        if(Integer.parseInt(MainData.get(YourID).get(taskEstimating))-everyOneEstm<5){
            compare="GOOD";
        } else {
            compare="HMM";
        }
    ...
    private void chooseFile(){
        ...
        if (rVal == JFileChooser.APPROVE_OPTION) {
            ...
            this.linkSaveFile = dir+"\\"+filename;
        }
    }
    ...
    private void saveFile(){
        ...
        try {
            FileWriter fw = new FileWriter(linkSaveFile);
            String temp = "";
            for (int i=0;i<MainData.size();i++){
                for(int j=0;j<MainData.get(i).size()-1;j++){
                    temp+=MainData.get(i).get(j)+",";
                }
                temp+=MainData.get(i).get(MainData.get(i).size()-1)+"\n";
            }
            fw.write(temp);
            fw.close();
        } catch (IOException e) {
            ...
            while (true) {
                ...
                try {
                    ...
                    if (sentence.startsWith("ok")) {
                        ...
                        clientGUI.MainData.get(id).add(value);
//                        setTaskLabel(clientGUI.TaskLabel, clientGUI.TaskEstimating);
                        ...
                        clientGUI.YourID = clientGUI.MainData.size()-1;
                    } else if (sentence.startsWith("NextTask")) {
                        //test
                        DefaultTableModel model = (DefaultTableModel) clientGUI.resultTable.getModel();
                        model.addRow(clientGUI.resultRow());

                        clientGUI.taskEstimating++;
                        clientGUI.estimatingTaskLabel.setText("Estimating task: "+ clientGUI.taskEstimating);
                        clientGUI.submitBtn.setEnabled(true);
                        ...
                        for(int i = 0; i< clientGUI.MainData.size(); i++){
                            for(int j = 0; j< clientGUI.MainData.get(0).size(); j++){
                                System.out.print(clientGUI.MainData.get(i).get(j)+" - ");
                            }
                            System.out.println(" ");
                        }
                        DefaultTableModel model = (DefaultTableModel) clientGUI.resultTable.getModel();
                        ...
                        // bật nút cho lưu file
                        clientGUI.saveFileBtn.setVisible(true);
                    }
                } catch (IOException ex) {
                    ex.printStackTrace();
                }
